...
const init = () => {
  var editor = document.getElementById("editor");
  editorContent = document.getElementById('resena-txt').value;
  editor.innerHTML = editorContent;
};
...
function renderGeneros(datos, generosLista){
  ...
  datos.forEach(item => {
    ...
    let element = ``;
    if(generosLista.indexOf(id) !== -1){
      element += `
          <div class="cont-genero">
            <div class="item-genero selected-genero" id="${id}">${nombre}</div>
            <div style="display:block;" class="close-genero" id="${id}">
  ...
  document.querySelectorAll(".item-genero").forEach((genero) => {
    ...
    if(genero.classList.contains("selected-genero") ){
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let autorID = AutorID.value;
    const autorOriginal = autorID;

    const generosString = document.getElementById('generos-libro');
    let generosLista = generosString.value.split("-").slice(1,-1);
    let generosListaINT = [];
    generosLista.forEach(genero => {
      generosListaINT.push(parseInt(genero));
    });
  ...
  const obtenerAutores = async () => {
    ...
    const obtenerGeneros = async () => {
      try {
        ...
        renderGeneros(generosDB, generosListaINT);
    ...
    const inputTituloOriginal = document.getElementById('titulo-libro-insertar');
    let tituloOriginal = inputTituloOriginal.value.trim();

    const formModificarLibro = document.getElementById('form-insertar-libro');
    formModificarLibro.addEventListener('submit', (e)=>{
        e.preventDefault();
        let errores = '';
        ...
        let id_libro = inputIDLibro.value;
        ...
        validarLibro(titulo, autorSeleccionado).then((returnedData) => {
          ...
          if(tituloOriginal.toLowerCase() !== titulo.toLowerCase()
            || autorSeleccionado !== autorOriginal){
            if(validacion == false){
          ...
          if(errores == ''){
              ...
              const insertarLibro = async () => {
                try {
                  const response = await fetch('/modificarLibro', {
                      method: 'POST',
                      body: formData,
                  });

                  if (response.ok) {
                      modalInfo('El libro se ha modificado con éxito.', "#00DD00");
                      setTimeout(()=>{

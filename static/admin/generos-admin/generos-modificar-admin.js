...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const inputNombreOriginal = document.getElementById('nombre-genero-insertar');
    let nombreOriginal = inputNombreOriginal.value.trim()
    ...
    formInsertarGenero.addEventListener('submit', (e)=>{
        ...
        let id_genero = document.getElementById('id_genero').value;
        ...
        validarGenero(nombre).then((returnedData) => {
          ...
          if(errores == ''){
            ...
            const modificarGenero = async () => {
              try {
                const response = await fetch('/modificarGenero', {
                    method: 'POST',
                    body: formData,
                });
    
                if (response.ok) {
                    modalInfo('El genero se ha modificado con éxito.', "#00DD00");
                    setTimeout(()=>{

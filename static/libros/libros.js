...
const filtrarGeneroBuscador = async (generos, buscador, tipoBusqueda) => {
  try {
      ...
      const response = await axios.post("/busquedaLibros", { data: datosEnviar }, {
          headers: {
...
function renderGeneros(datos){
  ...
  document.querySelectorAll(".item-genero").forEach((genero) => {
    genero.addEventListener("click", () => {
      ...
      filtrarGeneroBuscador(generosFiltrar, inputBuscador.value.trim(), tipoBusqueda);
...
async function filtrarGeneroUnico(id){
  ...
  try {
    ...
    const response = await axios.post("/filtrarGeneroUnico", { data: datosEnviar }, {
      headers: {
        "Content-Type": "application/json",
      },
    });
...
async function filtrarAutor(id) {
  try {
    ...
    const response = await axios.post("/filtrarAutor", { data: datosEnviar }, {
      headers: {
        "Content-Type": "application/json",
      },
    });
...
document.addEventListener("DOMContentLoaded", function () {
    const btnPagina = document.getElementById('pagLibros');
    const navItemPagina = document.getElementById('nav-itemLibros');
    ...
    const obtenerLibros = async () => {
      try {
        ...
        const response = await fetch("/obtenerLibros", requestOptions);
